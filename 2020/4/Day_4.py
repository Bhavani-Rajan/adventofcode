...
def is_valid_1(file_path):
    contents = read_input(file_path)
    list_input = contents_to_list(contents)
    list_passport = list_to_dict(list_input)
    ...
    for passport in list_passport:
        list_passport_keys = sorted(list(passport.keys()))
        if  (list_passport_keys == list_chk_1) or (list_passport_keys == list_chk_2):
            cnt = cnt + 1
    print(f"Number of Valid passports are : {cnt}\n")
...
if __name__ == "__main__":
    ...
    file_path="input.txt"
    is_valid_1(file_path)
    print("+++++ Part 2 +++++")
    file_path="example_2.txt"
    file_path="input.txt"
    is_valid_2(file_path)
